<?php
	...
	while (($json['data'] || $firstLoop) && $page*$pageSize <= $records){
		...
		."fields[game]=mapVersion,startTime,featuredMod&"
		."fields[player]=login&"
		."fields[featuredMod]=displayName&"
		."fields[map]=displayName&"
		."fields[mapVersion]=map,thumbnailUrlSmall&"
		.$nameFilter
		."sort=-startTime&"
		."page[number]=".$page."&"
		."page[size]=".$pageSize."";
	...
	foreach($games as $category=>$featuredMod){
		foreach($featuredMod as $featuredModId=>$thisGame){
			...
			$mapVersionId = $thisGame['relationships']['mapVersion']['data']['id'];
			if ($mapVersionId == null){
				continue;
			}
			$mapId = $includes['mapVersion'][$mapVersionId]['relationships']['map']['data']['id'];
			$mapPreview = $includes['mapVersion'][$mapVersionId]['attributes']['thumbnailUrlSmall'];
			$previews[$mapId] = $mapPreview;
			
			if (!array_key_exists($featuredModName, $topMaps)){
				$topMaps[$featuredModName] = [];
			}
			if (!array_key_exists($mapId, $topMaps[$featuredModName])){
				$topMaps[$featuredModName][$mapId] = 0;
			}
			$topMaps[$featuredModName][$mapId]++;
		}
	}
	
	// 3 : Ordering maps and trimming to $top, then display
	foreach($topMaps as $featuredModName=>$featuredMod){
		...
		foreach($featuredMod as $id=>$occurences){
			$mapName = $includes['map'][$id]['attributes']['displayName'];
